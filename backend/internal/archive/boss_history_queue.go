...
type BossHistoryWriter interface {
	SaveBossHistory(context.Context, vote.BossHistoryEntry) error
}
...
func NewBossHistoryQueue(cfg BossHistoryQueueConfig, writer BossHistoryWriter) *BossHistoryQueue {
	...
	return &BossHistoryQueue{
		...
		ch:           make(chan vote.BossHistoryEntry, bufferSize),
	}
}
...
func (q *BossHistoryQueue) Enqueue(entry vote.BossHistoryEntry) bool {
	select {
	case q.ch <- entry:
		return true
	default:
...
func (q *BossHistoryQueue) worker() {
	...
	for entry := range q.ch {
		...
		if err := q.writer.SaveBossHistory(ctx, entry); err != nil {
			xlog.L().Warn("archive boss history failed", xlog.Err(err))
		}
		cancel()
	}
}
