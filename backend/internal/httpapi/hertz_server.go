...
func NewHertzServer(addr string, options Options, tlsConfig *tls.Config) *server.Hertz {
	hlog.SetLogger(xlog.NewHertzLogger())

	serverOptions := []config.Option{server.WithHostPorts(addr)}
	if tlsConfig != nil {
		serverOptions = append(serverOptions, server.WithTLS(tlsConfig))
	}

	engine := server.Default(serverOptions...)
