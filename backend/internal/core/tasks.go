...
func (s *Store) ListTasksForPlayer(ctx context.Context, nickname string) ([]PlayerTask, error) {
	...
	for _, taskDef := range taskDefs {
		...
		var equipDetails []EquipmentRewardDetail
		for _, e := range taskDef.Rewards.EquipmentItems {
			def, err := s.getEquipmentDefinition(ctx, e.ItemID)
			if err != nil {
				equipDetails = append(equipDetails, EquipmentRewardDetail{ItemID: e.ItemID, Name: e.ItemID})
				continue
			}
			equipDetails = append(equipDetails, EquipmentRewardDetail{
				ItemID:               def.ItemID,
				Name:                 def.Name,
				Rarity:               def.Rarity,
				...
				ArmorPenPercent:      def.ArmorPenPercent,
				CritRate:             def.CritRate,
				CritDamageMultiplier: def.CritDamageMultiplier,
				PartTypeDamageSoft:   def.PartTypeDamageSoft,
				PartTypeDamageHeavy:  def.PartTypeDamageHeavy,
				PartTypeDamageWeak:   def.PartTypeDamageWeak,
			})
		}
		items = append(items, PlayerTask{
			...
			ClaimedAt:              claimedAt,
			EquipmentRewardDetails: equipDetails,
