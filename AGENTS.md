...
- GitHub Actions 当前构建并推送 `82.157.208.173:5000/long:latest`，再通过 SSH 登录 `x1sv` 执行 `docker pull` 和 `docker compose up -d` 部署。
...
- GitHub Actions 会构建并推送 `82.157.208.173:5000/long:latest`。
- GitHub Actions 会同步 `docker-compose.yml` 与服务器端部署 `.env`，然后 SSH 到 `x1sv` 执行：
