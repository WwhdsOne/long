...
describe('RoomSelector 紧凑卡片', () => {
    it('房间卡仅展示 Boss 均血，不再展示战斗力准入', () => {
        ...
        expect(source).toContain("if (room?.joinable !== true) return '暂不可切'")
        expect(source).toContain("return props.loggedIn && !props.switching && props.cooldownRemainingSeconds <= 0 && room?.joinable === true && !isCurrentRoom(room)")
    ...
    it('大厅房间列表保持三列，并把容器高度扩到至少能在五行内容纳 15 个房间', () => {
        ...
        expect(source).toContain('min-height: 94px;')
    })
})
