  ...
  <div class="admin-section">
    <section class="social-card admin-section-card boss-cycle-row">
      ...
      <div class="boss-cycle-row__right">
        ...
        <button class="nickname-form__submit" type="button" :disabled="saving || bossCycleEnabled"
                @click="enableBossCycle">
          开启循环
        </button>
        <button class="nickname-form__ghost" type="button" :disabled="saving || !bossCycleEnabled"
    ...
    <section class="social-card admin-section-card">
      ...
      <div class="boss-template-pagination">
        <button class="nickname-form__ghost" type="button" :disabled="templatePage <= 1" @click="goPrevPage">上一页
        </button>
        <span>第 {{ templatePage }} / {{ totalTemplatePages }} 页（每行 5 个）</span>
        <button class="nickname-form__ghost" type="button" :disabled="templatePage >= totalTemplatePages"
                @click="goNextPage">下一页
        </button>
      </div>
    </section>
    ...
    <div v-if="templateDialogOpen" class="dialog-mask">
      <section class="dialog-body">
        ...
        <div class="dialog-grid">
            ...
            <fieldset class="admin-fieldset">
              ...
              <div v-if="selectedCell" class="boss-editor-inspector">
                <div class="boss-editor-inspector__form">
                  ...
                  <label class="boss-editor-inspector__field">
                    <span>名称</span>
                    <input v-model="selectedCell.displayName" class="nickname-form__input" type="text"
                           placeholder="例如：眼核"/>
                  </label>
                  <label class="boss-editor-inspector__field">
                    <span>图片</span>
                    <input v-model="selectedCell.imagePath" class="nickname-form__input" type="text"
                           placeholder="/images/boss/eye.png"/>
                  </label>
                  <label class="boss-editor-inspector__field">
                    <span>血量</span>
                    <input v-model="selectedCell.maxHp" class="nickname-form__input" type="text" inputmode="numeric"
                           pattern="[0-9]*"/>
                  </label>
                  <label class="boss-editor-inspector__field">
                    <span>护甲</span>
          ...
          <div class="dialog-panel">
            ...
            <div class="admin-form admin-form--tight">
              <div v-for="(entry, index) in lootRows" :key="`${bossForm.id}-${index}-${entry.itemId}`"
                ...
                <input v-model="entry.dropRatePercent" class="nickname-form__input" type="number" min="0" max="100"
                       step="0.01" placeholder="掉落几率 %"/>
        ...
        <div class="dialog-footer">
          ...
          <button class="nickname-form__submit" type="button" :disabled="saving" @click="saveTemplateDesign">
            保存模板（含掉落与部位）
          </button>
        </div>
      </section>
    ...
    <div v-if="queueDialogOpen" class="dialog-mask">
      <section class="dialog-body dialog-body--queue">
            ...
            <ul class="queue-list">
              <li v-for="(entry, index) in queueTemplateEntries" :key="entry.templateId" class="queue-item">
                ...
                <div class="queue-item__actions">
                  <button class="nickname-form__ghost" type="button" :disabled="index === 0"
                          @click="moveQueueTemplate(index, -1)">上移
                  </button>
                  <button class="nickname-form__ghost" type="button"
                          :disabled="index === queueTemplateEntries.length - 1" @click="moveQueueTemplate(index, 1)">下移
                  </button>
          ...
          <div class="queue-panel">
            ...
            <ul class="queue-template-list">
              <li v-for="entry in bossTemplates" :key="entry.id" class="queue-item">
                ...
                <button class="nickname-form__ghost" type="button" :disabled="queueTemplateSet.has(entry.id)"
                        @click="addTemplateToQueue(entry.id)">
        ...
        <div class="dialog-footer">
          <button class="nickname-form__submit" type="button" :disabled="saving" @click="submitBossCycleQueue">
            保存循环队列
          </button>
        </div>
      </section>
