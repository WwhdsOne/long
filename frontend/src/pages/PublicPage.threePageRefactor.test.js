...
describe('PublicPage 三页前台边界', () => {
  ...
  it('战斗页只呈现 Boss 分区，不再显示常规按钮墙', () => {
    ...
    expect(battleSource).toContain("`boss-part:${zone.x}-${zone.y}`")
    expect(battleSource).toContain('class="boss-part-cell boss-zone-button"')
    expect(battleSource).toContain('boss-zone-button__label')
    expect(battleSource).toContain('zone.displayName')
    expect(battleSource).not.toContain('zone.assignedButton')
    expect(battleSource).not.toContain('displayedButtons.length === 0')
