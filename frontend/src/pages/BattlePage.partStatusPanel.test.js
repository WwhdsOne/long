...
describe('BattlePage 部位状态面板', () => {
    it('左侧新增可复用的部位状态面板，并接入剥皮、出血、护甲崩塌与末日标记状态', () => {
        ...
        expect(stateSource).toContain('const skinnerDurationSec = Math.max(0, Number(cs?.skinnerDurationByPart?.[key]) || Number(talentVisualState.value?.skinnerDurationByPart?.[key]) || 0)')
        expect(stateSource).toContain('const cachedDurationSec = Math.max(0, Number(vs.skinnerDurationByPart?.[partKey]) || 0)')
        expect(stateSource).toContain('const observedRemainingSec = Math.max(0, Math.ceil(endsAt - nowSec))')
        expect(stateSource).toContain('nextSkinnerDurationByPart[partKey] = incomingDurationSec || cachedDurationSec || observedRemainingSec')
        ...
        expect(stateSource).toContain("statusLabel: '变为弱点'")
        expect(stateSource).toContain('showProgress: false')
...
describe('BattlePage 全局状态面板', () => {
    it('左侧全局状态统一走可复用的 status-panel 列表，而不是保留专属卡片模板', () => {
        ...
        expect(battleSource).toContain('v-if="status.showProgress !== false" class="status-panel__bar"')
