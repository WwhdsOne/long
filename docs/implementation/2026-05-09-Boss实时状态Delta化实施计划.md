...
- 同批发布，不做双协议并行
- 先保证正确，再收包体
- `snapshot` 不是兜底常态，只允许首包和真异常恢复使用
- 同一轮公共广播里，同一只 Boss 必须复用同一份快照和同一个版本号
...
1. 服务端为公共 Boss 动态态维护“状态版本”而不是“发包版本”
2. 只有 Boss 运行态变化时才推进版本号
   - 普通点击导致 Boss 血量/部位变化
   - 挂机、流血 tick、魔法结算导致运行态变化
   - Boss 切换
3. `totalVotes`、在线人数、SSE 首包、同轮多客户端广播都不能单独推进 Boss 版本
4. 同一轮公共广播里：
   - 同一房间 / 同一 Boss 复用同一份公共快照
   - 同一房间 / 同一 Boss 复用同一个版本号
5. 复用现有 `sync_request`，但只在真异常时触发
   - 本地没有当前 `bossId` 的静态结构
   - `bossId` 不一致且无法直接切换
...
4. 旧包丢弃
5. 只有真异常才 `sync_request`
6. 允许同版本重复包只更新非 Boss 版本字段，例如 `totalVotes`
...
- 额外覆盖“同轮多客户端广播不重复推进版本”的测试
- 额外覆盖“SSE 首包和 snapshot 首包不推进版本”的测试
