...
> 状态：待评审
> 日期：2026-05-09
> 结论：本次只改 `Boss` 高频实时下行，采用“首包全量 + 高频 delta + 丢包自动重同步”的单协议切换方案；`room_state` 保持现状，不纳入本轮重构。
...
每个 `Boss` 动态状态维护单调递增 `version`。

要求：

- 同一 `bossId` 下严格递增
- 每次会改变公共 `Boss` 动态态的事件都推进版本号
- 新 `Boss` 生成后版本号从新基线开始
