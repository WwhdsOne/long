...
如果你调用了一些skill，请在输出prompt中说明你调用了哪些skill。
...
```bash
export CONSUL_ADDR=http://47.93.83.136:8500
export CONSUL_CONFIG_KEY=long-develop.config
```
...
```bash
rtk make backend-run        # Go 后端，默认 127.0.0.1:2333
rtk make frontend-dev       # Vite 前端，默认 localhost:5173
```
...
```bash
rtk make test               # 运行 Go 后端测试
rtk npm --prefix frontend run test   # 运行 Vitest 前端测试
```
...
```bash
rtk go -C backend test ./internal/vote/...   # 测试 vote 包
rtk go -C backend test -run TestXxx ./...    # 运行指定测试函数
```
...
\- Prefer `rtk read`, `rtk grep`, `rtk find`, `rtk git status`, `rtk git diff` for inspection.
\- Do not use `python3 << 'PYEOF'` just to read files.
\- For large files, use `rtk read <file> -l aggressive` first.
\- Use normal Python only when actual computation or transformation is required.
